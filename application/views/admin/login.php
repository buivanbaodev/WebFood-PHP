...
<body>
    <div class="conatiner">
        ...
        <p class="text-center mb-4">Đăng nhập để tiếp tục</p>
        <?php
      ...
      ?>
        <form action="<?php echo base_url().'admin/login/authenticate' ;?>" name="loginform" id="loginform"
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control my-2" name="username" id="username" placeholder="Tên đăng nhập">
                <span></span>
            </div>
            <?php echo form_error('username'); ?>
            <div class="form-group">
                <label for="password">Điền mật khẩu</label>
                <input type="password" class="form-control my-2" name="password" id="password" placeholder="Mật khẩu">
                <span></span>
            </div>
            <?php echo form_error('password'); ?>
            <button type="submit" class="btn btn-info btn-block mb-2">Đăng nhập</button>
        </form>
    </div>
    <script>
        ...
        const validate = () => {
            ...
            if(usernameVal === "") {
                setErrorMsg(username, 'Tên đăng nhập không để trống');
            } else {
                setSuccessMsg(username);
            }
            if(passwordVal === "") {
                setErrorMsg(password, 'Mật khẩu không để trống');
            } else {
