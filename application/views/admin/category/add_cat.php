<div class="container shadow-container">
    <h2 class="p-2 text-center">Thêm danh mục nhà hàng</h2>
    <form action="<?php echo base_url().'admin/category/create_category';?>" class="container my-3" method="POST" id="myForm">
        <div class="form-group">
            <label for="category">Loại</label>
            <input type="text" id="category" class="form-control" placeholder="" name="category">
            <?php echo form_error('category'); ?>
            <span></span>
        </div>
        <button type="submit" class="btn btn-primary mr-2">Lưu</button>
        <a class="btn btn-secondary" href="<?php echo base_url().'admin/category/index';?>">Hủy bỏ</a>
    </form>
</div>
...
<script>
    ...
    const validate = () => {
        ...
        //
        if (categoryVal === "") {
            setErrorMsg(category, 'Loại không để trống');
        } else if (categoryVal.length <= 3 || categoryVal.length >= 16) {
            setErrorMsg(category, 'Độ dài nên từ 3 đến 15"');
        } else {
