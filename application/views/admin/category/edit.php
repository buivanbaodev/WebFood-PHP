<div class="container shadow-container">
    <h2 class="p-2 text-center">Sửa thể loại nhà hàng "<?php echo $cat['c_name'];?>"</h2>
    <form action="<?php echo base_url().'admin/category/edit/'.$cat['c_id'];?>" class="container" method="POST" id="myForm">
        ...
        <button type="submit" class="btn btn-primary mr-2">Thực hiện thay đổi</button>
        <a class="btn btn-secondary" href="<?php echo base_url().'admin/category/index';?>">Trở lại</a>
    </form>
</div>
<script>
    ...
    const validate = () => {
        ...
        if (categoryVal === "") {
            setErrorMsg(category, 'Thể loại không để trống');
        } else if (categoryVal.length <= 4 || categoryVal.length >= 16) {
            setErrorMsg(category, 'Độ dài phải từ 5 đến 15"');
        } else {
