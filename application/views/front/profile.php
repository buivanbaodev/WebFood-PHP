            ...
            <form action="<?php echo base_url().'profile/edit/'.$loggedUser['user_id']?>" method="POST">
                ...
                <div class="py-2">
                    ...
                    <div class="py-3 pb-4 border-bottom">
                        <button type="submit" class="btn btn-primary mr-3">Lưu thay đổi</button>
                        <a href="<?php echo base_url().'home' ?>" class="btn border button">Hủy bỏ</a>
                    </div>
                </div>
            </form>
        </div>
                ...
                <div class="py-2">
                    <div>
                        <input type="password"
                            ...
                            name="cPassword" placeholder="Mật khẩu hiện tại" value="<?php echo set_value('cPassword')?>">
                        <?php echo form_error('cPassword'); ?>
                    </div>
                    <div>
                        <input type="password"
                            class="bg-light form-control mb-2 <?php echo (form_error('nPassword') != "") ? 'is-invalid' : '';?>"
                            name="nPassword" placeholder="Mật khẩu mới" value="<?php echo set_value('nPassword')?>">
                        <?php echo form_error('nPassword'); ?>
                    </div>
                    <div>
                        <input type="password"
                            class="bg-light form-control mb-2 <?php echo (form_error('nRPassword') != "") ? 'is-invalid' : '';?>"
                            name="nRPassword" placeholder="Xác nhận mật khẩu"
                            value="<?php echo set_value('nRPassword')?>">
                        <?php echo form_error('nRPassword'); ?>
                    </div>
                    <div class="py-3 pb-4 border-bottom">
                        <button type="submit" class="btn btn-primary mr-3 mb-2">Lưu thay đổi</button>
                        <a href="<?php echo base_url().'home' ?>" class="btn border button">Hủy bỏ</a>
                    </div>
                </div>
            </form>
