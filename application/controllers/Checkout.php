       ...
       $user = $this->User_model->getUser($u_id);
        ...
        if($this->cart->total_items() <= 0) {
            ...
            if($this->form_validation->run() == true) { 
                ...
                if($order) {
                    $this->session->set_flashdata('success_msg', 'Cảm ơn! Đơn hàng của bạn đã được đặt thành công!');
                       redirect(base_url().'orders');
                } else {
                     $data['error_msg'] = "Gửi đơn đặt hàng không thành công, vui lòng thử lại.";
                }
            }
