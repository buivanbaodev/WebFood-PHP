...
class Profile extends CI_Controller {
    ...
    public function edit($id) {
        ...
        if(empty($user)) {
            $this->session->set_flashdata('error', 'Không tìm thấy người dùng');
        ...
        if($this->form_validation->run() == true) { 
            ...
            $this->session->set_flashdata('success', 'Người dùng được cập nhật thành công');
        ...
        if($this->form_validation->run() == true) { 
            ...
            if(password_verify($cPassword, $user['password']) == true) {
                if($nPassword != $nRPassword) {
                    $this->session->set_flashdata('pwd_error', 'Mật khẩu không khớp');
                    redirect(base_url(). 'profile/index');
                }else {
                    $formArray['password'] = password_hash($this->input->post('nPassword'), PASSWORD_DEFAULT);

                    $this->User_model->update($id,$formArray);
                    $this->session->set_flashdata('pwd_success', 'Đã cập nhật mật khẩu thành công');
                    redirect(base_url(). 'profile/index');
                }
            }else {
                $this->session->set_flashdata('pwd_error', 'Mật khẩu cũ của bạn không chính xác');
                redirect(base_url(). 'profile/index');
            }
        }else {
            $data['user'] = $user; 
