...
class Orders extends CI_Controller {
    ...
    public function deleteOrder($id) {
        ...
        if(empty($order)) {
            $this->session->set_flashdata('error_msg', 'Không tìm thấy đơn hàng');
            redirect(base_url().'orders');
        }

        $this->Order_model->deleteOrder($id);

        $this->session->set_flashdata('success_msg', 'Đơn đặt hàng của bạn đã được hủy thành công');
        redirect(base_url().'orders');
    ...
    public function invoice($id) {
        ...
        if($u_id == $user['user_id']) {
            if($order['status'] == 'closed') {
                ...
                $this->session->set_flashdata('error_msg', 'Đơn đặt hàng của bạn vẫn chưa hoàn thành');
                redirect(base_url().'orders');
            }
        } else {
            $this->session->set_flashdata('error_msg', 'Bạn đang truy cập dữ liệu đơn đặt hàng sai');
            redirect(base_url().'orders');
        }        
    }
}